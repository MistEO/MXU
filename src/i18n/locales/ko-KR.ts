export default {
  ...
  mirrorChyan: {
    ...
    gotIt: '확인',
    // 폴백 업데이트 복구 안내 (지난 자동 업데이트 실패로 폴백, 수동 덮어쓰기 미완료)
    fallbackRecovery: {
      title: '지난 업데이트가 완료되지 않았습니다',
      message:
        '지난 자동 업데이트가 정상적으로 완료되지 않았습니다. 새 버전 파일은 아래 폴더에 압축 해제되었지만 프로그램 디렉터리에 적용되지 않아 작업 목록이 누락되거나 기능이 정상 작동하지 않을 수 있습니다.\n이 프로그램을 종료한 후 폴백 폴더 안의 모든 파일을 프로그램 디렉터리에 복사하여 덮어쓴 다음 다시 실행하세요.',
      pathLabel: '폴백 폴더 경로:',
      openFolder: '폴백 폴더 열기',
      done: '수동으로 완료했습니다',
      hint: '참고: 덮어쓸 때 MaaEnd.exe도 함께 교체하세요. 완료 후 폴백 폴더를 삭제하거나 "수동으로 완료했습니다"를 클릭하면 다시 표시되지 않습니다.',
    },
