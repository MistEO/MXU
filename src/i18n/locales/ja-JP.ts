export default {
  ...
  mirrorChyan: {
    ...
    gotIt: '了解',
    // フォールバック更新の復旧案内（前回の自動更新が失敗しフォールバック、手動での上書き未完了）
    fallbackRecovery: {
      title: '前回の更新が未完了です',
      message:
        '前回の自動更新が正常に完了しませんでした。新しいバージョンのファイルは下記のフォルダーに展開されましたが、プログラムディレクトリへの反映が済んでいないため、タスク一覧が欠落したり機能が正常に動作しない場合があります。\n本アプリを終了してから、フォールバックフォルダー内のすべてのファイルをプログラムのディレクトリにコピーして上書きし、再度起動してください。',
      pathLabel: 'フォールバックフォルダーのパス：',
      openFolder: 'フォールバックフォルダーを開く',
      done: '手動で完了しました',
      hint: 'ヒント：上書きの際は MaaEnd.exe も一緒に置き換えてください。完了後はフォールバックフォルダーを削除するか、「手動で完了しました」を押すと再表示されません。',
    },
