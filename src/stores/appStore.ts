...
import {type AccentColor, applyTheme, clearCustomAccents, type CustomAccent, registerCustomAccent, resolveThemeMode, unregisterCustomAccent,} from '@/themes';
import type {MxuConfig, RecentlyClosedInstance} from '@/types/config';
import {defaultMirrorChyanSettings, defaultScreenshotFrameRate, defaultWindowSize,} from '@/types/config';
import type {ActionConfig, Instance, OptionDefinition, OptionValue, SelectedTask,} from '@/types/interface';
...
// 重新导出类型供外部使用
export type {DownloadProgress, DownloadStatus, InstallStatus, JustUpdatedInfo, Language, LogEntry, LogType, PageView, ScheduleExecutionInfo, TaskRunStatus, Theme, UpdateInfo,} from './types';
...
export const useAppStore = create<AppState>()(
    subscribeWithSelector(
        (set, get) => ({
          ...
          setMaxLogsPerInstance: (value) => set({
            maxLogsPerInstance:
                Math.max(100, Math.min(10000, Math.floor(value)))
          }),
          ...
          updateCustomAccent: (id, accent) => {
            set((state) => ({
                  customAccents: state.customAccents.map(
                      (a) => (a.id === id ? accent : a)),
                }));
            // 先移除旧的，再注册新的
          ...
          removeCustomAccent: (id) => {
            ...
            if (accent) {
              ...
              set((state) => ({
                    customAccents:
                        state.customAccents.filter((a) => a.id !== id),
                  }));
          ...
          reorderCustomAccents: (oldIndex, newIndex) => {
            set((state) => {
              ...
              if (oldIndex < 0 || newIndex < 0 || oldIndex >= next.length ||
          ...
          createInstance: (name) => {
            ...
            if (pi) {
              ...
              pi.task.forEach((task) => {
                ...
                // 检查任务是否支持默认控制器
                const isControllerCompatible = !task.controller ||
                    task.controller.length === 0 ||
                    !!(defaultControllerName &&
                       task.controller.includes(defaultControllerName));
                ...
                defaultTasks.push({
                  ...
                  taskName: task.name,
                  enabled:
                      isControllerCompatible,  // 不兼容默认控制器的任务不勾选
                  optionValues,
          ...
          removeInstance: (id) => set((state) => {
            ...
            if (instanceToClose) {
              const closedRecord: RecentlyClosedInstance = {
                ...
                tasks: instanceToClose.selectedTasks.map(
                    (t) => ({
                      id: t.id,
                      taskName: t.taskName,
              ...
              // 添加到列表头部，并限制最大条目数
              newRecentlyClosed = [closedRecord, ...state.recentlyClosed].slice(
                  0, MAX_RECENTLY_CLOSED);
            }
          ...
          addTaskToInstance: (instanceId, task) => {
            ...
            set((state) => ({
                  ...
                  animatingTaskIds:
                      [...state.animatingTaskIds, newTask.id],  // 加入动画列表
                }));
          },

          // 添加延迟任务到实例（保留向后兼容，内部调用 addMxuSpecialTask）
          addSleepTaskToInstance:
              (instanceId: string, sleepTime: number = 5) => {
          ...
          addMxuSpecialTask: (
            ...
            for (const [optionKey, optionDef] of Object.entries(
              ...
              if (optionDef.type === 'input') {
                ...
                const isOn = defaultCase === 'Yes' ||
                    defaultCase === optionDef.cases[0]?.name;
              ...
              set((state) => ({
                    instances: state.instances.map(
                        (i) => i.id === instanceId ? {
                          ...i,
                          selectedTasks:
                              i.selectedTasks.filter((t) => t.id !== taskId)
                        } :
                                                     i,
                        ),
                  })),
          ...
          toggleTaskEnabled: (instanceId, taskId) => set(
              (state) => ({
                instances: state.instances.map(
                    (i) => i.id === instanceId ? {
                      ...i,
                      selectedTasks: i.selectedTasks.map(
                          (t) =>
                    ...
                    return {
                      ...
                      selectedTasks: i.selectedTasks.map((t) => {
                        ...
                        const newOptionValues =
                            {...t.optionValues, [optionKey]: value};
                        ...
                        if (pi?.option) {
                          ...
                          if (optDef &&
                              (optDef.type === 'switch' ||
                               optDef.type === 'select' || !optDef.type) &&
                              'cases' in optDef) {
                            let selectedCase;

                            if (optDef.type === 'switch') {
                              const isChecked =
                                  value.type === 'switch' && value.value;
                              selectedCase =
                                  findSwitchCase(optDef.cases, isChecked);
                            } else {
                              const caseName = value.type === 'select' ?
                                  value.caseName :
                                  optDef.cases?.[0]?.name;
                              selectedCase = optDef.cases?.find(
                                  (c) => c.name === caseName);
                            }

                            // 初始化嵌套选项（如果尚未初始化）
                            if (selectedCase?.option &&
                                selectedCase.option.length > 0) {
              ...
              set((state) => ({
                    instances: state.instances.map(
                        (i) => i.id === instanceId ? {
                          ...i,
                          selectedTasks:
                              i.selectedTasks.map((t) => ({...t, enabled})),
                        } :
                                                     i,
                        ),
                  })),
              ...
              set((state) => ({
                    instances: state.instances.map(
                        (i) => i.id === instanceId ? {
                          ...i,
                          selectedTasks:
                              i.selectedTasks.map((t) => ({...t, expanded})),
                        } :
                                                     i,
                        ),
                  })),
          ...
          duplicateTask: (instanceId, taskId) => {
            ...
            const taskIndex =
                instance.selectedTasks.findIndex((t) => t.id === taskId);
            ...
            if (originalTask.customName) {
              ...
              const taskDef = state.projectInterface?.task.find(
                  (t) => t.name === originalTask.taskName);
              const langKey = getInterfaceLangKey(state.language);
              const originalLabel =
                  state.resolveI18nText(taskDef?.label, langKey) ||
                  taskDef?.name || originalTask.taskName;
            ...
            set({
              instances: state.instances.map(
                  (i) => i.id === instanceId ? {...i, selectedTasks: tasks} : i,
                  ),
            });
          },
          ...
          moveTaskUp: (instanceId, taskId) =>
              set((state) => ({
                    instances: state.instances.map((i) => {
                      ...
                      const taskIndex =
                          i.selectedTasks.findIndex((t) => t.id === taskId);
              ...
              set((state) => ({
                    instances: state.instances.map((i) => {
                      ...
                      const taskIndex =
                          i.selectedTasks.findIndex((t) => t.id === taskId);
                      if (taskIndex === -1 ||
                          taskIndex >= i.selectedTasks.length - 1)
                        return i;
          ...
          duplicateInstance: (instanceId) => {
            ...
            const newInstance: Instance = {
              ...
              selectedTasks: sourceInstance.selectedTasks.map(
                  (t) => ({
                    ...t,
                    id: generateId(),
                    optionValues: {...t.optionValues},
              ...
              isRunning: false,
              preAction: sourceInstance.preAction ?
                  {...sourceInstance.preAction} :
                  undefined,
            };
          ...
          removeAnimatingTaskId: (taskId) =>
              set((state) => ({
                    animatingTaskIds:
                        state.animatingTaskIds.filter((id) => id !== taskId),
                  })),
          ...
          removeAnimatingTabId: (tabId) =>
              set((state) => ({
                    animatingTabIds:
                        state.animatingTabIds.filter((id) => id !== tabId),
                  })),
          ...
          importConfig: (config) => {
            ...
            const snapshotTaskNames =
                new Set(config.interfaceTaskSnapshot || []);
            ...
            if (pi) {
              pi.task.forEach((task) => {
                ...
                if (!snapshotTaskNames.has(task.name) ||
                    savedNewTaskNames.has(task.name)) {
            ...
            const validOptionNames =
                new Set(pi?.option ? Object.keys(pi.option) : []);
            ...
            const instances: Instance[] = config.instances.map((inst) => {
              // 记录被过滤掉的无效任务
              const invalidTasks =
                  inst.tasks.filter((t) => !validTaskNames.has(t.taskName));
              if (invalidTasks.length > 0) {
                loggers.config.warn(
                    `实例 "${inst.name}" 中有 ${
                        invalidTasks.length} 个无效任务被移除:`,
            ...
            const accentColor =
                (config.settings.accentColor as AccentColor) || 'deepsea';
            ...
            set({
              ...
              mirrorChyanSettings:
                  config.settings.mirrorChyan || defaultMirrorChyanSettings,
              ...
              connectionPanelExpanded:
                  config.settings.connectionPanelExpanded ?? true,
              screenshotPanelExpanded:
                  config.settings.screenshotPanelExpanded ?? true,
              screenshotFrameRate: config.settings.screenshotFrameRate ??
            ...
            if (minimizeToTray) {
              import('@tauri-apps/api/core').then(({invoke}) => {invoke('set_minimize_to_tray', {enabled : minimizeToTray}).catch((err) => {
          ...
          setSelectedController: (
              instanceId, controllerName) => set((state) => {
          ...
          setInstanceSavedDevice: (instanceId, savedDevice) =>
              set((state) => ({
                    instances: state.instances.map(
                        (i) => (i.id === instanceId ? {...i, savedDevice} : i)),
                  })),

          setInstancePreAction: (
              instanceId: string, action: ActionConfig|undefined) =>
              set((state) => ({
                    instances: state.instances.map(
                        (i) =>
          ...
          setCachedWin32Windows: (windows) =>
              set({cachedWin32Windows: windows}),
          ...
          restoreBackendStates: (states) => set((currentState) => {
            ...
            const updatedInstances = currentState.instances.map((instance) => {
              ...
              if (backendState) {
                ...
                const isRunning =
                    backendState.isRunning && backendState.taskIds.length > 0;
                return {
                  ...instance,
                  isRunning,
                };
            ...
            for (const [instanceId, state] of Object.entries(
                     states.instances)) {
          ...
          toggleDashboardView: () =>
              set((state) => ({dashboardView: !state.dashboardView})),
          ...
          setMirrorChyanChannel: (channel) => set(
              (state) => ({
          ...
          setUpdateCheckLoading: (loading) =>
              set({updateCheckLoading: loading}),
          ...
          reopenRecentlyClosed: (id) => {
            ...
            const closedInstance =
                state.recentlyClosed.find((i) => i.id === id);
            ...
            const newInstance: Instance = {
              ...
              selectedTasks: closedInstance.tasks.map(
                  (t) => ({
                    id: generateId(),
                    taskName: t.taskName,
                    customName: t.customName,
          ...
          removeFromRecentlyClosed: (id) => set(
              (state) => ({
                recentlyClosed: state.recentlyClosed.filter((i) => i.id !== id),
              })),
          ...
          appendPendingTaskId: (instanceId, taskId) => set(
              (state) => ({
                instancePendingTaskIds: {
                  ...state.instancePendingTaskIds,
                  [instanceId]: [
                    ...(state.instancePendingTaskIds[instanceId] || []), taskId
                  ],
                },
              })),
          ...
          advanceCurrentTaskIndex: (instanceId) =>
              set((state) => ({
                    instanceCurrentTaskIndex: {
                      ...state.instanceCurrentTaskIndex,
                      [instanceId]:
                          (state.instanceCurrentTaskIndex[instanceId] || 0) + 1,
                    },
                  })),
          ...
          setScheduleExecution: (instanceId, info) =>
              set((state) => ({
                    scheduleExecutions: info ?
                        ...
                        Object.fromEntries(
                            Object.entries(state.scheduleExecutions)
...
function generateConfig(): MxuConfig {
  ...
  return {
    ...
    interfaceTaskSnapshot:
        state.projectInterface?.task.map((t) => t.name) || [],
    // 保存用户尚未查看的新增任务
